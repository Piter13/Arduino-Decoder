...
char CODE[]="AFbE0180"; /*Domyślny Kod Hex*/
int TIMES = 1;
int DISP = 250;
...
void setup() {
  ...
  Serial.println("Team - Piotr Margański, Mateusz Wiśniewski, Aleksy Wołowiec"); //Jednorazowe wysłanie tekstu
  printError(starting);
  readCode();
  delay(3000);
}
...
int action_listener( int button_number){  
  if(button_number == 0){   
    ...
    return 1;
    /* Naciśnięcie w trakcie procesu łamania kodu przycisku S1
    powoduje wstrzymanie łamania, a kolejne  naciśnięcie wznawia łamanie*/
  }
  
  if(button_number ==1){ 
     /*Naciśnięcie przycisku S2 – wtedy ponownie łamany jest dotychczasowy kod  
    naciskanie przycisku S2 w trakcie łamania nie powinna powodować żadnej akcji.*/
    if(completed){
      Serial.println("Button2 - Action"); 
